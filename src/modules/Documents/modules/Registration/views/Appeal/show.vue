...
onMounted(async () => {
  loading.value = true
  await regAppeal.actionGetById({ id: route.params.id })
  setTimeout(() => {
    loading.value = false
  }, 500)
})
...
<template>
  <div class="incoming-show-view h-full">
    ...
    <template v-else>
      <layout-with-tabs
        :title="regAppeal.detailModel.outgoing_number"
        :preview-detail="regAppeal.detailModel.__copy_prototype"
        :object-id="regAppeal.detailModel.id"
