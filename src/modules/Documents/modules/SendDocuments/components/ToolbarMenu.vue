...
<template>
  ...
  >
    <div class="flex flex-col p-1 gap-y-1">
      <template v-for="menu in unSelectedMenuItems">
        ...
        >
          <base-iconify
            ...
            class="text-greyscale-500 mr-2 hover:text-primary-dark"
          />
          {{ t(menu.label) }}
        </div>
      </template>
      ...
      <div
        ...
        @click="dialog = !dialog"
      >
        <base-iconify :icon="SettingsIcon" class="text-greyscale-500 mr-2" />
        {{ t('customize-menu') }}
      </div>
    </div>
  </base-overlay-panel>
  ...
  <customize-menu-dialog
    ...
    @emit:reset-toolbar-menu="onChangeDocType(sdStore.SD_TOOLBAR_MENU_LIST[0])"
  />
</template>
