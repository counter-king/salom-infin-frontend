...
const getBranches = (newFilter = {}) => {
  ...
  axiosConfig
    ...
    .then(response => {
      ...
      const newBranches = (Array.isArray(results) ? results: []).map(user => ({...user, position: user?.position?.name}));
...
const searchBranches = search => {
  const newFilter = { ...filter.value, page: 1, search };
  getBranches(newFilter);
};
...
const getFirstPageBranches = () => {
  getBranches(defaultFilter);
};
...
const saveChanges = e => {
  ...
  localStorage.setItem('settings-structure-branches', newHeaders);
  settingsOverlay.value.toggle(e);
};
const resetHeaders = e => {
  const newHeaders = headers.value.map(header => ({ ...header, is_active: true }));
  headers.value = newHeaders;
  localStorage.setItem('settings-structure-branches', JSON.stringify(newHeaders));
  settingsOverlay.value.toggle(e);
};
const initHeaders = () => {
  const list = JSON.parse(localStorage.getItem('settings-structure-branches'));
    ...
    <div class="flex items-center gap-2">
      <span class="p-input-icon-left">
        ...
        <InputText
          ...
          @update:modelValue="searchBranches"
          placeholder="Поиск"
          size="small"
          type="text"
          />
      ...
      >
        <template #body="{ field, data }">
          <Branch
            ...
            :branches="branches"
            :field="field"
            :getFirstPageBranches="getFirstPageBranches"
...
<style>
.branches-table th:first-child, td:first-child {
  border-radius: 12px 0 0 12px;
}
.branches-table th:last-child, td:last-child {
