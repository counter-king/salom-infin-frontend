...
const positionEdit = () => {
   ...
   if(name_uz && name_ru && code) {
      ...
      axiosConfig
         .patch(`/positions/${positionId}/`, { name_ru, name_uz, code })
         .then(response => {
            const data = response?.data;
            const status = response?.status;
            if(status === 200) {
               const newPositions = props?.positions.map(position => {
               ...
               dispatchNotify(null, t('updated-position'), 'success');
   ...
   axiosConfig
      ...
      .then(response => {
         if(response?.status === 204) {
            ...
            dispatchNotify(null, t('deleted-position'), 'success')
...
const updateCondition = value => {
   ...
   const positionId = props?.data?.id;
   const name_uz = props?.data?.name_uz;
   const name_ru = props?.data?.name_ru;
   const code = new Date().toISOString();
   axiosConfig
      .patch(`positions/${positionId}/`, { is_active: value?.value, name_ru, name_uz, code })
      .then(response => {
         const data = response?.data;
         const status = response?.status;
         if(status === 200) {
...
<template>
   ...
   <Dialog
      ...
      <div class="flex flex-col items-center pb-10 pt-4">
         ...
         <h2 class="text-center font-semibold text-3xl text-gray-900 p-0 mt-6">{{ t('delete-position') }}?</h2>
         <p class="text-center py-0 px-6 mt-2 text-gray-400">
            {{ t('delete-text-position') }}
         </p>
      </div>
      <template #footer>
         <div class="flex justify-end">
