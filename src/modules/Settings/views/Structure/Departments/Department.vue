...
const departmentEdit = () => {
   ...
   if(name_uz && name_ru && code && companyId) {
      ...
      axiosConfig
         .patch(`/departments/${departmentId}/`, { name_ru, name_uz, condition, code, parnet_code: 0, company: companyId })
         .then(response => {
            ...
            if(status === 200) {
               const newDepartments = props?.departments.map(department => {
                  if(department?.id === departmentId) {
                     return { ...data, branch: data?.company?.name };
                  } else {
                     return department;
                  }
               });
               dispatchNotify(null, t('updated-department'), 'success');
   ...
   axiosConfig
      ...
      .then(response => {
         if(response?.status === 204) {
            ...
            dispatchNotify(null, t('deleted-department'), 'success')
...
const openEditModal = () => {
   const code = props.data?.code || 0;
   const newCompany = props.data?.company  || '';
   company.value = newCompany;
   editDepartment.value = {...props.data, code};
...
<template>
   ...
   <template v-else>
      <router-link :to="`/settings/structure/departments/${data.id}`">
         <div class="px-4">
   ...
   <Dialog
      ...
      <div class="flex flex-col items-center pb-10 pt-4">
         ...
         <h2 class="text-center font-semibold text-3xl text-gray-900 p-0 mt-6">{{ t('delete-department') }}?</h2>
         <p class="text-center py-0 px-6 mt-2 text-gray-400">
            {{ t('delete-text-department') }}
         </p>
      </div>
      <template #footer>
         <div class="flex justify-end">
