      ...
      axiosConfig
         ...
         .then(response => {
            ...
            if(status === 200) {
               ...
               dispatchNotify(null, t('updated-document-type'), 'success');
               editVisible.value = false;
               props.setDocumentTypes(newDocumentTypes);
            } else {
               dispatchNotify(null, t('not-updated-document-type'), 'error');
            }
         })
         .catch(() => {
            dispatchNotify(null, t('not-updated-document-type'), 'error');
   ...
   axiosConfig
      ...
      .then(response => {
         if(response?.status === 204) {
            ...
            dispatchNotify(null, t('deleted-document-type'), 'success')
            props.getFirstPageDocumentTypes();
         } else {
            dispatchNotify(null, t('not-deleted-document-type'), 'error')
         }
      })
      .catch(() => {
         dispatchNotify(null, t('not-deleted-document-type'), 'error')
...
<template>
   ...
   <Dialog
      ...
      <div class="flex flex-col items-center pb-10 pt-4">
         ...
         <h2 class="text-center font-semibold text-3xl text-gray-900 p-0 mt-6">{{ t('delete-document-type') }}?</h2>
         <p class="text-center py-0 px-6 mt-2 text-gray-400">
            {{ t('delete-document-type-text') }}
         </p>
      </div>
      <template #footer>
         <div class="flex justify-end">
