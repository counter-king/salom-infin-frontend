...
import CreateDocumentTitle from './CreateDocumentTitle.vue';
...
const getDocumentTitles = (newFilter = {}) => {
  loading.value = true;
  filter.value = newFilter;
  const { page, page_size, search } = newFilter;
  const params = `?page=${page}${page_size ? '&page_size=' + page_size : ''}${search ? '&search=' + search : ''}`;
  axiosConfig
    .get(`document-titles/${params}`)
    .then(response => {
      const results = response?.data?.results;
      const newCount = response?.data?.count;
      const newDocumentTitles = Array.isArray(results) ? results: [];
      documentTitles.value = newDocumentTitles;
...
const searchDocumentTitles = search => {
  const newFilter = { ...filter.value, page: 1, search };
  getDocumentTitles(newFilter);
};
...
const getFirstPageDocumentTitles = () => {
  getDocumentTitles(defaultFilter);
};
const setDocumentTitles = newDocumentTitles => {
  documentTitles.value = newDocumentTitles;
}
...
const saveChanges = e => {
  ...
  localStorage.setItem('settings-document-titles', newHeaders);
  settingsOverlay.value.toggle(e);
};
const resetHeaders = e => {
  const newHeaders = headers.value.map(header => ({ ...header, is_active: true }));
  headers.value = newHeaders;
  localStorage.setItem('settings-document-titles', JSON.stringify(newHeaders));
  settingsOverlay.value.toggle(e);
};
const initHeaders = () => {
  const list = JSON.parse(localStorage.getItem('settings-document-titles'));
...
<template>
    ...
    <div class="flex items-center gap-2">
      <span class="p-input-icon-left">
        ...
        <InputText
          ...
          @update:modelValue="searchDocumentTitles"
          placeholder="Поиск"
          size="small"
          type="text"
          />
  ...
  <div class="document-titles-table">
    <DataTable
      :loading="loading"
      :pt="tableConfig"
      :value="documentTitles"
      row-hover
      scrollable
      ...
      <Column
        ...
        <template #body="{ field, data }">
          <DocumentTitle
            ...
            :field="field"
            :getFirstPageDocumentTitles="getFirstPageDocumentTitles"
...
<style>
.document-titles-table th:first-child, td:first-child {
  border-radius: 12px 0 0 12px;
}
.document-titles-table th:last-child, td:last-child {
