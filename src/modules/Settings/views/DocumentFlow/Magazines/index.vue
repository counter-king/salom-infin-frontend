...
const getJournals = (newFilter = {}) => {
  ...
  axiosConfig
    ...
    .then(response => {
      ...
      const newJournals = Array.isArray(results) ? results: [];
...
const getFirstPageJournals = () => {
  getJournals(defaultFilter);
};
const setJournals = newJournals => {
  journals.value = newJournals;
}
...
const saveChanges = e => {
  ...
  localStorage.setItem('settings-magazines', newHeaders);
  settingsOverlay.value.toggle(e);
};
const resetHeaders = e => {
  const newHeaders = headers.value.map(header => ({ ...header, is_active: true }));
  headers.value = newHeaders;
  localStorage.setItem('settings-magazines', JSON.stringify(newHeaders));
  settingsOverlay.value.toggle(e);
};
const initHeaders = () => {
  const list = JSON.parse(localStorage.getItem('settings-magazines'));
      ...
      <Column
        ...
        <template #body="{ field, data }">
          <Journal
            :data="data"
            :field="field"
            :getFirstPageJournals="getFirstPageJournals"
            :journals="journals"
...
<style>
.journals-table th:first-child, td:first-child {
  border-radius: 12px 0 0 12px;
}
.journals-table th:last-child, td:last-child {
