...
import CreateDeliveryType from './CreateDeliveryType.vue';
...
const searchDeliveryTypes = search => {
  const newFilter = { ...filter.value, page: 1, search };
  getDeliveryTypes(newFilter);
};
...
const getFirstPageDeliveryTypes = () => {
  getDeliveryTypes(defaultFilter);
};
const setDeliveryTypes = newDeliveryTypes => {
  deliveryTypes.value = newDeliveryTypes;
}
...
const saveChanges = e => {
  ...
  localStorage.setItem('settings-delivery-types', newHeaders);
  settingsOverlay.value.toggle(e);
};
const resetHeaders = e => {
  const newHeaders = headers.value.map(header => ({ ...header, is_active: true }));
  headers.value = newHeaders;
  localStorage.setItem('settings-delivery-types', JSON.stringify(newHeaders));
  settingsOverlay.value.toggle(e);
};
const initHeaders = () => {
  const list = JSON.parse(localStorage.getItem('settings-delivery-types'));
...
onMounted(() => {
  getFirstPageDeliveryTypes();
  initHeaders();
});
</script>
<template>
  <div class="flex mb-5 justify-between items-center">
    <h1 class="text-2xl font-bold text-primary-900">{{ t('deliver-type') }}</h1>
    <div class="flex items-center gap-2">
      <span class="p-input-icon-left">
        ...
        <InputText
          ...
          @update:modelValue="searchDeliveryTypes"
          placeholder="Поиск"
          size="small"
          type="text"
          />
  ...
  <div class="delivery-types-table">
    <DataTable
      :loading="loading"
      :pt="tableConfig"
      :value="deliveryTypes"
      ...
      <Column
        ...
        <template #body="{ field, data }">
          <DeliveryType
            ...
            :field="field"
            :getFirstPageDeliveryTypes="getFirstPageDeliveryTypes"
  ...
  <OverlayPanel ref="settingsOverlay" :pt="overlayConfig">
    ...
    <div class="flex justify-end border-t bg-greyscale-50 py-3 pr-5 pl-8">
      <Button @click="resetHeaders" class="p-button p-component shadow-button font-medium flex justify-center rounded-full text-[14px] py-[6px] px-4 bg-white text-primary-900 border-transparent">Сбросить</Button>
...
<style>
.delivery-types-table th:first-child, td:first-child {
  border-radius: 12px 0 0 12px;
}
.delivery-types-table th:last-child, td:last-child {
