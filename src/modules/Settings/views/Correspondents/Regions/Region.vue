...
const props = defineProps({ data: Object, field: String, getFirstPageRegions: Function, regions: Array, setRegions: Function });
   ...
   if(name_uz && name_ru && code) {
      ...
      axiosConfig
         ...
         .then(response => {
            ...
            if(status === 200) {
               const newRegions = props?.regions.map(region => {
               ...
               dispatchNotify(null, t('updated-region'), 'success');
   ...
   axiosConfig
      ...
      .then(response => {
         if(response?.status === 204) {
            ...
            dispatchNotify(null, t('not-deleted-region'), 'error')
         }
      })
      .catch(() => {
         dispatchNotify(null, t('not-deleted-region'), 'error')
...
const updateCondition = value => {
   ...
   const regionId = props?.data?.id;
   axiosConfig
      .patch(`regions/${regionId}/`, { is_active: value?.value })
      .then(response => {
         const data = response?.data;
         const status = response?.status;
         if(status === 200) {
            ...
            dispatchNotify(null, t('status-not-updated'), 'error');
         }
      })
      .catch(() => {
         dispatchNotify(null, t('status-not-updated'), 'error');
...
<template>
   ...
   <Dialog
      ...
      <div class="flex flex-col items-center pb-10 pt-4">
         ...
         <h2 class="text-center font-semibold text-3xl text-gray-900 p-0 mt-6">{{ t('delete-region') }}?</h2>
