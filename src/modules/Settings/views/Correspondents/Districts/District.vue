...
import { dialogConfig, selectConfig, menuConfig } from './config';
   ...
   if(name_uz && name_ru && regionId) {
      ...
      axiosConfig
         ...
         .then(response => {
            ...
            if(status === 200) {
               const newDistricts = props?.districts.map(district => {
...
const updateCondition = value => {
   ...
   const districtId = props?.data?.id;
   axiosConfig
      .patch(`districts/${districtId}/`, { is_active: value?.value })
      .then(response => {
         const data = response?.data;
         const status = response?.status;
         if(status === 200) {
...
<template>
   ...
   <template v-else-if="field === 'region'">
      <span class="text-sm font-medium">{{ data[field].name }}</span>
   </template>
   <template v-else>
