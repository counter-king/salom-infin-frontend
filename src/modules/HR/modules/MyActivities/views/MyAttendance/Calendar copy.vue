...
watch(()=> currentMonthFirstDate.value, async () => {
  await attendanceStore.getMyAttendanceList({ start_date: dayjs(currentMonthFirstDate.value).format('YYYY-MM-DD'), end_date: dayjs(currentMonthFirstDate.value).endOf('month').format('YYYY-MM-DD') })
}, { immediate: true })
    ...
    <div class="flex flex-col bg-white  rounded-2xl overflow-hidden shadow-button mt-4">
        ...
        <div class="grid grid-cols-7">
          ...
          <template v-for="(day, index) in calendarDaysWithAttendance" :key="index">
            <!-- cell line-through; -->
            <!-- v-if="day?.attendance?.type != USER_STATUS_CODES.ADDITIONAL_DAY_OFF && day?.attendance?.type != USER_STATUS_CODES.LATE || !day?.workDay" -->
            <div
