...
const create = async () => {
  ...
  try {
    ...
    await agreementCategory.createCategories(createModel.value)
    await agreementCategory.getCategories()
    dispatchNotify(null, 'Категория создана', COLOR_TYPES.SUCCESS)
...
const update = async () => {
  ...
  try {
    ...
    await agreementCategory.updateCategories(updateModel.value.id, updateModel.value)
    await agreementCategory.getCategories()
    dispatchNotify(null, 'Категория изменен', COLOR_TYPES.SUCCESS)
...
const deleteColumn = async (id) => {
  ...
  try {
    await agreementCategory.deleteCategories(id)
    await agreementCategory.getCategories()
    dispatchNotify(null, 'Категория удален', COLOR_TYPES.SUCCESS)
  }
  catch(error) {
...
<template>
  <div class="category-view">
    <action-toolbar title="Категория документа">
      <template #filters>
        <base-button
    ...
    >
      ...
      <template #description="{ data }">
        <span class="font-medium text-greyscale-900 line-clamp-2">{{ data.description ?? '-' }}</span>
      </template>
    ...
    >
      <template #header>
        <div class="flex-1 truncate mr-2">
          <h1 class="text-xl font-semibold truncate">Создать категория документа</h1>
        </div>
      </template>
    ...
    >
      <template #header>
        <div class="flex-1 truncate mr-2">
          <h1 class="text-xl font-semibold truncate">Изменить категория документа</h1>
        </div>
      </template>
