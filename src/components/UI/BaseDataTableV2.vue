...
<template>
  <DataTable
    ...
    :pt="
    {
      ...
      bodyRow: { class: ['cursor-pointer text-sm font-medium text-greyscale-900 last:border-b last:border-b-greyscale-100'] },
    ...
    >
      <template #body="{ index, data }">
        <slot
          ...
          :data="data"
        >
          <span class="text-sm font-medium text-greyscale-500">{{
              (paginationStore.pageSize * paginationStore.page + 1) - (paginationStore.pageSize - index) }}</span>
        </slot>
      </template>
    </Column>
